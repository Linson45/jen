...
class TourOperatorDashboard extends BaseService {
  ...
  TourOperatorDashboard = (pageNo,date) => {
    ...
    return this.axios.get(`/tour-operator/${userId}/dashboard?page=${pageNo}&size=6&startDate=${fromDate}&endDate=${toDate}`, {
      headers: {
        "authorization": `Bearer ${token}`,
        'Accept': 'application/json',
  ...
  tourOperatorBookingById = (BookingId) => {
    ...
    return this.axios.get(`/tour-operator/booking/${BookingId}/dashboard`, {
      headers: {
        "authorization": `Bearer ${token}`,
        'Accept': 'application/json',
        'Content-Type': 'application/json'
...
export default new TourOperatorDashboard();